...
class routerConfigsTask extends sfBaseTask
{
  ...
  protected function execute($arguments = array(), $options = array())
  {
     ...
     */
    $ipf_config = '/etc/ipf.rules';
    $ipnat_config = '/etc/ipnat.rules';
    ...
    foreach ($identities as $itemIdentity)
    {
      ...
      {
        $ipnat_rules []= sprintf(
          'rdr %s from %s/32 to 0.0.0.0/0 port = 80 -> 127.0.0.1 port 3128',
          $config['router']['local_iface'], $itemIdentity['ip']
          );
        $ipfilter_rules []= sprintf(
          'pass in quick on %s from %s/32 to any keep state',
          $config['router']['local_iface'], $itemIdentity['ip']
        );
      }
      else
    ...
    $squid_lists_dir = '/usr/local/etc/squid/lists';
